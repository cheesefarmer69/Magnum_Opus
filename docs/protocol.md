...
| 26 | `ACTIE_ALARM` | **Drukknop-roulette-alarm**: chaotisch, irritant zoemer-alarm van ~10,4 s (eindige noten-tabel, 1,8–3,8 kHz, speelt zichzelf uit). Puur geluid — raakt de LED-staat niet. Wordt **vervroegd gestopt door actie 18** (`ACTIE_KNOP_UIT`): de roulette-afhandeling stuurt die bij een druk of time-out. Gewone `commando_message_v2` (FIFO/ACK). |
| 27 | `ACTIE_STORM` | **Storm-baan**: alle 7 LEDs continu **cyaan** `CRGB(0,200,255)` — een kleur die door geen enkele andere actie gebruikt wordt. Gezet door "Sync toestanden + LEDs" op de uren van het trekkende storm-event. Solid, FIFO/ACK. |
| 28 | `ACTIE_BLIKSEM` | **Bliksem-inslag** (storm + bliksem-dynamiek): grillige **gele** dubbel-flits (~1,3 s, `BLIKSEM_MS`), daarna **auto-terug naar `ACTIE_STORM`** (cyaan) — een inslag gebeurt per definitie op een storm-paal, dus de paal herstelt zichzelf zonder rebuild. Valt onder de S3b-latch (tekent door tijdens de BLE-scan). FIFO/ACK. |
...
| `bommen/keuze`     | dashboard ↔ Node-RED | `"aot"\|"maki"` (retained) — **gekozen bommen-track** voor de minigame "Bommen vermijden". De dashboard-dropdown "Track" publiceert; Node-RED zet `global.bommenKeuze` + laadt de bijhorende `bommenTijdlijn`/`bommenTrack` uit `global.bommenTracks` |
| `config/bag`       | dashboard ↔ Node-RED | `{"quota":{"verplaatsing":4,"toestand":2,"wereld":2,"drukknop":1},"vrijeSlots":1,"blokgrootte":10,"geenHerhaling":true,"maxHoogOpRij":2,"drukknopMin":10,"actief":false,"drukknopEvents":["tijdbom_speler"]}` (retained) — **bag-systeem**: de categorie-mix per blok van 10 events. `actief:false` = het oude pure-tier-gedrag. `drukknop` is een **virtuele** categorie (verwijst naar `drukknopEvents`), want geen enkel event heeft die categorie. Zie `docs/spel/event-systeem.md` |
| `config/flow`      | dashboard ↔ Node-RED | `{"aanloopS":5,"tempoOverride":0,"legendaryFreq":50,"profiel":"vlak"}` (retained) — **live flow-parameters**: aanloop/aankondiging in seconden (clamp 3–15), handmatige tempo-override (0 = auto, wereld-events bepalen het tempo) en legendary-frequentie |
