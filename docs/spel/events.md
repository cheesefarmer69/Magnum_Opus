...
| `voorwaarde`    | nee       | `min` / `max` / `of` / `priem` / `wissel` / `geen` — beweging-controle na de reactietijd. Bij `of`: geldig als `voor` exact `x` óf exact `y` is. Bij **`priem`**: geldig als het aantal stappen een priemgetal is (**2, 3, 5, 7, 11, 13, 17, 19, 23**; 1 telt niet mee) — anders status `GEEN PRIEMGETAL` met `delta = max(0, stappen − afstand tot het dichtstbijzijnde priemgetal)`. |
...
| `knop_gok` | `variant: "dubbel"\|"plusmin"`            | **Drukknop-gok** ("Dubbel of niets" / "Plus 5 of min 3"). Kiest een **vrije** drukknop-paal (niet ontmantel/middernacht/mnZone/roulette/verdeel), zet `global.gokPaal` + `gokVariant` en **armt de knop (actie 17)**. Wie drukt gokt voor **iedereen die op dat moment op die paal staat**; kans **overdag 50 %**, **'s nachts 40 %**. `dubbel` = uren ×2 of naar 0; `plusmin` = +5 of −3 (vloer 0). Afhandeling in "Knop-verwerking"; verloopt de duratie zonder druk, dan ontwapent "Verouder effecten" de knop. |
| `bipolair` | —                                         | **Bipolair beestje** op de doelwit-uren. Volgt de middernachtpoort: **open = houdt van EVEN** verplaatsingen (LED oranje), **dicht = ONEVEN** (LED limoen). Wie op dat uur eindigt met de juiste pariteit krijgt **+1**, met de verkeerde **−1**. Na **4 humeurwissels** verdwijnt het. **Max 3 tegelijk**. State: `global.bipolair = [{paal, even, wissels}]`. |
