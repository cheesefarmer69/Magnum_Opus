...
- **Geplande bommen / beat-sync (juli 2026, "Bommen vermijden"):** `MSG_BOM` is **v2** (15 B: + signed `wacht_ms` + `seq`; 10 B-v1 wordt geaccepteerd). Cue's gaan `LEAD` (1,2 s) vooraf de lucht in; de master herzendt phase-locked met per poging een **vers herberekende signed rest-wacht** (`verwerkBomQueue`, loop-task-privé — bewust géén lock/ACK, dedupe via `seq` op de slave); de slave vuurt op `dueMs` met **`actieStartMs = dueMs`** als anker (late bezorging kort de ramp in, doofmoment blijft op de beat). **Deploy-volgorde: eerst alle slaves, dan masters, dan flows** — een oude slave vuurt een v2-frame direct én bij elke resend opnieuw af. De **flourish-wis op 66,0 s in `bommen_engine` mag nooit actie 0 worden** (wist geplande bommen — gebruik actie 16 helderheid 0); Stop/einde gebruiken wél actie 0 (bewust wissen). Show-gate-uitzondering S3b: tijdens de BLE-scan wordt er tóch gelatcht voor `ACTIE_BOM`/`ACTIE_KLOKSLAG` (continu her-getekend, self-healing); revert via `-DBOM_SHOW_TIJDENS_SCAN=0` in `firmware/Slave/platformio.ini`. Zie `docs/protocol.md` §0/§2 + `docs/spel/bommen.md`.
