...
- De **volledige actie-tabel** (0–28) staat in `docs/protocol.md §2` — dé bron. Nieuw juli 2026: **26 `ACTIE_ALARM`** (drukknop-roulette, ~10,4 s chaotische melodietabel, gestopt door actie 18), **27 `ACTIE_STORM`** (cyaan solid, storm-baan), **28 `ACTIE_BLIKSEM`** (gele flikker ~1,3 s, auto-terug naar **STORM**-cyaan, in de S3b-latch). Voeg nooit een actie/berichttype toe zonder die tabel + de slave/master-firmware samen bij te werken. Acties **22 `ACTIE_KNOP_GOED`** / **23 `ACTIE_KNOP_FOUT`** = korte groen/rood-flits (auto-stop na `KNOP_FLITS_MS`) + positief/negatief zoemerdeuntje; FIFO/ACK (geldig gemaakt in `verwerkCommandos()`), voor de drukknop-events-modus + knoppendans. Actie **24 `ACTIE_ONTPLOFFING`** = witte knal → uitdovende rode strobe (`ONTPLOF_MS` 1,6 s) + dalende sirene-sweep; gestuurd bij een mislukte tijdbom-ontmanteling én bij een afgelopen bom-teller. Geen struct-/`msg_type`-wijziging → **alleen slaves herflashen**. ⚠️ Een nieuwe actie moet óók in de **ACK-whitelist** in `verwerkCommandos()` — staat ze daar niet, dan wordt ze geweigerd (ACK-status 1) en nooit uitgevoerd.
...
- **Storm / bliksem / drukknop roulette / gelijke verdeling / middernacht-uitbreiding (juli 2026):** drie nieuwe events + twee sim-instelbare dynamieken (`sim/spel-config` → `drukknopRouletteAan`/`bliksemAan`, checkboxes in het simulator-Spelinstellingen-paneel). Nieuwe partij-globals (`storm`, `stormRichting`, `mnZone`, `verdeelPaal`, `rouletteActief`, `rouletteCooldownTot`) staan in `resetPartij` → **container-herstart** nodig bij settings.js-wijziging. De storm schuift 1 uur per event op in **Verouder effecten** (die heeft nu **4 outputs** — out4 → Route commando voor bliksem/knop-uit-one-shots); schade + doorkruis-straf in **Verifieer beweging**; LED via **Sync** (`gewenst=27`, poort wint). **`mnZone`** (buurpalen van de poort) telt overal als poort mee: oversteek-scoring, poort-bewaker, STIL-straf, M10-gate, LED, doelwit-exclusie; een oversteek die op een zone-lid **landt** (23→1) mag wél. De **bag-drukknop** is een id-lijst `bagConfig.drukknopEvents` (`tijdbom_speler`,`gelijke_verdeling`) en die events zijn **uitgesloten** uit hun basis-categorie-pool (niet dubbel trekbaar). Roulette draait op de 1 s-ticker (`roulette_tick_fn`): alleen op een drukknop-paal **waar iemand op staat**, 10 s, niemand drukt → **iedereen −10%** (ceil, vloer 0, geen sterfte), cooldown 60 s.
- **Bommen-muziek-stop + track-keuze (juli 2026-fix):** de engine-stop draait op de eigen vlag **`bommenMuziekAan`** die **bewust NIET in `resetPartij`** staat (resetPartij nult `bommenGestart` vóór de engine-tick — daardoor bleef de muziek vroeger spelen bij een handmatige Stop, en werd ook de scan-herstel overgeslagen). `bommen_keuze_apply` forceert bij een lege `bommenTracks` (boot-race met de retained `bommen/keuze`, onceDelay 0,7 s) **niet** meer naar "aot" maar bewaart de keuze; de `[CONFIG]`-node past hem daarna toe.
- **Geplande bommen / beat-sync (juli 2026, "Bommen vermijden"):** `MSG_BOM` is **v2** (15 B: + signed `wacht_ms` + `seq`; 10 B-v1 wordt geaccepteerd). Cue's gaan `LEAD` (1,2 s) vooraf de lucht in; de master herzendt phase-locked met per poging een **vers herberekende signed rest-wacht** (`verwerkBomQueue`, loop-task-privé — bewust géén lock/ACK, dedupe via `seq` op de slave); de slave vuurt op `dueMs` met **`actieStartMs = dueMs`** als anker (late bezorging kort de ramp in, doofmoment blijft op de beat). **Deploy-volgorde: eerst alle slaves, dan masters, dan flows** — een oude slave vuurt een v2-frame direct én bij elke resend opnieuw af. De **sfeer-golf-wis in `bommen_engine` (na elk `tl.sfeer`-venster) mag nooit actie 0 worden** (wist geplande bommen — gebruik actie 16 helderheid 0); Stop/einde gebruiken wél actie 0 (bewust wissen). Show-gate-uitzondering S3b: tijdens de BLE-scan wordt er tóch gelatcht voor `ACTIE_BOM`/`ACTIE_KLOKSLAG` (continu her-getekend, self-healing); revert via `-DBOM_SHOW_TIJDENS_SCAN=0` in `firmware/Slave/platformio.ini`. Zie `docs/protocol.md` §0/§2 + `docs/spel/bommen.md`.
- **Reactietijd-sfx (`sfxReactie`, juli 2026):** een event-config-veld met **enkel de bestandsnaam** van een sfeer-effect dat **tijdens de reactietijd** speelt. `Kies event` (`c6a0000000000008`) hangt `sound-effect/<categorie>/<sfxReactie>` **achteraan** de afroep-segmenten (net als de bestaande generieke wereld-`woosh.wav` vooraan). Gekoppeld: **bomaanslag** (`sound-effect/wereld-events/bomaanslag.wav`, bang), **tornado** (`toestanden/tornado.wav`), **portalen** (`toestanden/portalen.wav`). Config-only (in de `[CONFIG]`-injects) + één push-regel in `Kies event` → **`deploy-flows.ps1`**, geen firmware. Zie `pi/audio-player/audio/sound-effect/README.md`.
- **Tweeling-verbroken cue (`tweelingVerbrokenCue`, juli 2026):** de eind-afroep `events/afgelopen/tweeling_verbroken.wav` ("de tweelingen zijn verbroken") speelt bij **elke** verbreking van een tweelingband (tweeling loopt niet op duratie af, dus **niet** via `audioAfgelopen`). De TW6-hereniging (`Verifieer beweging`, `c6a0000000000027`) én de dood-propagatie (`tweelingDood` in `settings.js`) zetten `global.tweelingVerbrokenCue=true`; **`Verouder effecten`** (`c6a0000000000012`) pikt de vlag op de gewone afgelopen-emissie op en reset ze. Nieuwe global in `resetPartij` → **container-herstart** nodig (settings.js), plus `deploy-flows.ps1` voor de flow-nodes.
